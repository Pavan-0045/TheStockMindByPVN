...
import AddProductLayout from "./AddProductLayout.jsx";

export default function AddProductForm({ onAddCustomField, onClose }) {
  ...
  return (
    <FormLayout
      ...
      size="large"
      showCustom={true}
      customText="Add Custom Field"
      showUpload={true}
      uploadText="Bulk Upload"
      onCustom={onAddCustomField}
      onClose={onClose}
    >
      {/* FORM */}
      <form
        onSubmit={handleSubmit(onSubmit)}
        className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-y-4 gap-x-4 mt-5 min-h-[600px]"
      >
        <CharInput
          label="Product Name"
          name="productName"
        ...
        <NumCharInput
          ...
          rule={{ required: "GRN Number is required" }}
          error={errors.grn}
          register={register}
        />
        ...
        <div className="col-span-1 sm:col-span-2 xl:col-span-3 grid grid-cols-1 lg:grid-cols-3 gap-6 min-w-0">
          ...
          {/* SUBMIT BUTTON */}
          <div className="col-span-1 sm:col-span-3 xl:col-span-3 mb-6">
